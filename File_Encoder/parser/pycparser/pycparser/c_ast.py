...
class Node(object):
    ...
    def changevar(self, oldvar, newvar):
        if self.attr_names:
            vlist = [getattr(self, n) for n in self.attr_names]
            for i in range(0, len(vlist)):
                if(vlist[i] == oldvar):
                    setattr(self, self.attr_names[i], newvar)

        for (child_name, child) in self.children():
            child.changevar(oldvar, newvar)

    def getvars(self):
        if(self.__class__.__name__ == 'ParamList'):
            for (child_name, child) in self.children():
                vlist = [getattr(child, n) for n in child.attr_names]
                for i in vlist:
                    istr = str(i)
                    if((istr != 'None') and (istr != '[]')):
                        print(istr)
